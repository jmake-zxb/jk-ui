...
export function toThousands(num: any) {
  return num?.toString().replace(/\d+/, (n: any) => {
...
export function numberFormat(num: number) {
  return num < 1000
    ? toThousands(num)
    : `${toThousands((num / 1000).toFixed(1))}k`;
}
...
// 头像
export const getDefaultIcon = () => `/${window.MaxKB.prefix}/favicon.ico`;

export function isAppIcon(url: string | undefined) {
  return url === getDefaultIcon() ? '' : url;
}
...
export function cutFilename(filename: string, num: number) {
  ...
  const suffix =
    lastIndex === -1 ? '' : filename.slice(Math.max(0, lastIndex + 1));
  return `${filename.slice(0, Math.max(0, num - suffix.length - 1))}.${suffix}`;
}
...
export const resetUrl = (url: string, defaultUrl?: string) => {
  if (url && url.startsWith('./')) {
    return `${window.MaxKB.prefix}/${url.slice(2)}`;
  }
