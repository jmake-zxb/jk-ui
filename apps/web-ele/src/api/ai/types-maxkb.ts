...
import type { Dict } from '#/api/ai/types-common';
...
export class ChatRecordManage {
  ...
  append_answer(
    ...
    if (chunk_answer || reasoning_content) {
      const set_index =
        index === undefined ? this.chat.answer_text_list.length - 1 : index;
      let card_list = this.chat.answer_text_list[set_index];
      if (!card_list) {
        card_list = [];
        this.chat.answer_text_list[set_index] = card_list;
      }
      const answer_value = card_list.find(
        (item) => item.real_node_id === real_node_id,
      );
      const content = answer_value
        ? answer_value.content + chunk_answer
        : chunk_answer;
  ...
  appendChunk(chunk: Chunk) {
    ...
    let n = this.node_list.find(
      (item) => item.real_node_id === chunk.real_node_id,
    );
    if (n) {
      ...
      n = {
        ...
        reasoning_content_buffer:
          (chunk.reasoning_content && [...chunk.reasoning_content]) || [],
  ...
  closeInterval() {
    ...
    const last_index = this.findIndex(
      this.chat.answer_text_list,
      (item) =>
        (item.length === 1 && item[0].content === '') || item.length === 0,
      'last',
    );
    if (last_index > 0) {
  ...
  get_run_node() {
    ...
    const run_node = this.node_list.find(
      (item) =>
    ...
    if (run_node) {
      ...
      if (
        ...
        answer_text_list_index =
          none_index === -1 ? this.chat.answer_text_list.length : none_index;
      } else {
        const none_index = this.findIndex(
          this.chat.answer_text_list,
          (item) =>
        ...
        answer_text_list_index =
          none_index === -1
            ? this.chat.answer_text_list.length - 1
            : none_index;
      }
...
export const ChatManagement = {
  chatMessageContainer: {} as Dict<ChatRecordManage>,

  addChatRecord(chat: chatType, ms: number, loading?: Ref<boolean>) {
  ...
  append(chatRecordId: string, content: string, reasoning_content?: string) {
  ...
  appendChunk(chatRecordId: string, chunk: Chunk) {
    const chatRecord = this.chatMessageContainer[chatRecordId];
    if (chatRecord) {
  ...
  clean() {
    for (const key of Object.keys(this.chatMessageContainer)) {
      if (this.chatMessageContainer[key].is_close) {
        Reflect.deleteProperty(this.chatMessageContainer, key);
      }
    }
  },
