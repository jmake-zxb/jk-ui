...
<template>
  <div
    ...
    class="public-chat-embed"
    :style="[shellStyle, backgroundStyle]"
  >
    <header class="public-chat-embed__header" :style="headerStyle">
      <ElButton
        v-if="showHistory"
        class="public-chat-embed__icon-button"
        text
        @click="drawerVisible = true"
      >
        <ElIcon><Menu /></ElIcon>
      </ElButton>
      <div class="public-chat-embed__brand">
        <div class="public-chat-embed__avatar">
          <img
            v-if="applicationDetail.icon && isValidUrl(applicationDetail.icon)"
            :src="applicationDetail.icon"
            alt=""
          />
          <ElIcon v-else :size="18"><ChatRound /></ElIcon>
        </div>
    ...
    <main class="public-chat-embed__main">
      <AiChat
        :key="chatKey"
        class="public-chat-embed__chat"
    ...
    <ElDrawer
      ...
      size="78%"
      :with-header="false"
      class="public-chat-embed__drawer"
    >
...
.public-chat-embed {
  --public-chat-embed-header-height: 48px;
...
.public-chat-embed__header {
  display: grid;
  flex: 0 0 var(--public-chat-embed-header-height);
...
.public-chat-embed__brand span {
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}

.public-chat-embed__avatar {
  display: flex;
  flex: 0 0 auto;
  align-items: center;
  justify-content: center;
...
.public-chat-embed__icon-button {
  color: inherit;
}

.public-chat-embed__main {
  flex: 1;
  min-height: 0;
}

.public-chat-embed__chat :deep(.ai-chat__operate) {
  padding-top: 12px;
}
