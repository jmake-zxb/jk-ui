...
const videoCascaderRef = ref<InstanceType<typeof NodeCascader>>();
...
function normalizeDialogueType(value: unknown) {
  const current = stringValue(value, 'NODE');
  return current === 'WORKFLOW' ? 'WORKFLOW' : 'NODE';
}
...
function normalizeNodeData(value: unknown): NodeData {
  ...
  return {
    ...
    dialogue_type: normalizeDialogueType(source.dialogue_type),
    ...
    video_list: Array.isArray(source.video_list)
      ? cloneDeep(source.video_list)
      : ['start-node', 'video'],
  };
}
...
async function validate() {
  try {
    if (formData.value.model_id_type === 'reference') {
      ...
      throw validationError('请选择视频理解模型');
    }

    if (!hasReferenceValue(formData.value.video_list)) {
      throw validationError('请选择视频变量');
    }
    await videoCascaderRef.value?.validate?.();
...
<template>
  ...
  >
    <ElForm
      :model="formData"
      class="workflow-video-understand-node"
      label-position="top"
      @submit.prevent
    >
      <section class="workflow-video-understand-node__panel">
        <ElFormItem label="视频理解模型" required>
          <template #label>
            <div class="workflow-video-understand-node__form-label">
              <span>视频理解模型</span>
              <ElSelect
          ...
          <div class="workflow-video-understand-node__model-row">
            <LocalModelSelect
        ...
        <ElFormItem required>
          <template #label>
            <div class="workflow-video-understand-node__label-with-tip">
              <span>视频</span>
              <FileTooltip />
            </div>
          </template>
          <NodeCascader
            ref="videoCascaderRef"
        ...
        <ElFormItem label="系统提示词">
          <template #label>
            <div class="workflow-video-understand-node__form-label">
              <span class="workflow-video-understand-node__label-with-tip">
                系统提示词
                <ElTooltip
                  content="定义模型角色、回答边界和输出风格。"
                  placement="right"
                >
                  <span class="workflow-video-understand-node__tip">?</span>
                </ElTooltip>
              </span>
              <ElButton
                size="small"
          ...
          <template #label>
            <div class="workflow-video-understand-node__form-label">
              <span class="workflow-video-understand-node__label-with-tip">
                用户提示词
                <ElTooltip
                  content="描述希望模型从视频中理解、提取或回答的内容，支持 {{节点.字段}} 引用。"
                  placement="right"
                >
                  <span class="workflow-video-understand-node__tip">?</span>
                </ElTooltip>
      ...
      <section
        ...
        class="workflow-video-understand-node__panel"
      >
        <ElFormItem>
          <template #label>
            <div class="workflow-video-understand-node__form-label">
      ...
      <section class="workflow-video-understand-node__panel">
        <div class="workflow-video-understand-node__switch-row">
          <div>
            <strong>输出思考</strong>
            <small>开启后按标记提取模型思考过程</small>
          </div>
          <div class="workflow-video-understand-node__switch-actions">
            <ElTooltip
        ...
        <div
          v-if="showConversationControls"
          class="workflow-video-understand-node__switch-row"
        >
          <div>
            <strong>返回内容</strong>
...
.workflow-video-understand-node {
  --video-understand-space-2xs: 2px;
  --video-understand-space-xs: 4px;
  --video-understand-space-sm: 6px;
  --video-understand-space-md: 8px;
  --video-understand-radius: 6px;

  display: grid;
  gap: var(--video-understand-space-sm);
}

.workflow-video-understand-node__panel {
  display: grid;
  gap: var(--video-understand-space-sm);
  padding: var(--video-understand-space-sm) var(--video-understand-space-md);
  overflow: visible;
  background: var(--el-fill-color-extra-light);
  border: 1px solid var(--el-border-color-lighter);
  border-radius: var(--video-understand-radius);
}

.workflow-video-understand-node :deep(.el-form-item) {
  margin-bottom: var(--video-understand-space-sm);
}

.workflow-video-understand-node :deep(.el-form-item:last-child) {
  margin-bottom: 0;
}

.workflow-video-understand-node :deep(.el-form-item__label) {
  margin-bottom: var(--video-understand-space-2xs);
  font-size: var(--el-font-size-extra-small);
  line-height: 16px;
}

.workflow-video-understand-node :deep(.el-input__wrapper) {
  min-height: 28px;
}

.workflow-video-understand-node :deep(.el-textarea__inner) {
  min-height: 0 !important;
  padding-block: var(--video-understand-space-xs);
  line-height: 18px;
}

.workflow-video-understand-node__form-label,
.workflow-video-understand-node__label-with-tip,
.workflow-video-understand-node__switch-row,
.workflow-video-understand-node__switch-actions {
  display: flex;
  gap: var(--video-understand-space-sm);
  align-items: center;
  min-width: 0;
...
.workflow-video-understand-node__form-label .el-select {
  width: 92px;
}

.workflow-video-understand-node__model-row {
  display: grid;
...
.workflow-video-understand-node__tip {
  display: inline-flex;
  align-items: center;
...
.workflow-video-understand-node__switch-row strong,
.workflow-video-understand-node__switch-row small {
  display: block;
}
...
.workflow-video-understand-node__switch-row small {
  margin-top: var(--video-understand-space-2xs);
  font-size: 11px;
  font-weight: 400;
  line-height: 14px;
