...
async function validate() {
  try {
    if (formData.value.model_id_type === 'reference') {
      ...
      throw validationError('请选择文生视频模型');
    }
...
<template>
  ...
  >
    <ElForm
      :model="formData"
      class="workflow-text-to-video-node"
      label-position="top"
      @submit.prevent
    >
      <section class="workflow-text-to-video-node__panel">
        <ElFormItem label="文生视频模型" required>
          <template #label>
            <div class="workflow-text-to-video-node__form-label">
              <span>文生视频模型</span>
              <ElSelect
          ...
          <div class="workflow-text-to-video-node__model-row">
            <LocalModelSelect
        ...
        <ElFormItem label="提示词" required>
          <template #label>
            <div class="workflow-text-to-video-node__form-label">
              <span class="workflow-text-to-video-node__label-with-tip">
                提示词
                <ElTooltip
                  content="描述要生成的视频内容，支持 {{节点.字段}} 引用。"
                  placement="right"
                >
                  <span class="workflow-text-to-video-node__tip">?</span>
                </ElTooltip>
              </span>
              <ElButton
                size="small"
      ...
      <section
        ...
        class="workflow-text-to-video-node__panel"
      >
        <div class="workflow-text-to-video-node__switch-row">
          <div>
            <strong>返回内容</strong>
...
.workflow-text-to-video-node,
.workflow-text-to-video-node__panel {
  display: grid;
  gap: 6px;
}
...
.workflow-text-to-video-node :deep(.el-form-item) {
  margin-bottom: 6px;
}

.workflow-text-to-video-node :deep(.el-form-item:last-child) {
  margin-bottom: 0;
}

.workflow-text-to-video-node :deep(.el-form-item__label) {
  margin-bottom: 2px;
  font-size: var(--el-font-size-extra-small);
  line-height: 16px;
}

.workflow-text-to-video-node :deep(.el-input__wrapper) {
  min-height: 28px;
}

.workflow-text-to-video-node :deep(.el-textarea__inner) {
  min-height: 0 !important;
  padding-block: 4px;
  line-height: 18px;
}

.workflow-text-to-video-node__form-label,
.workflow-text-to-video-node__label-with-tip,
.workflow-text-to-video-node__switch-row {
  display: flex;
  gap: 6px;
  align-items: center;
  min-width: 0;
...
.workflow-text-to-video-node__form-label .el-select {
  width: 92px;
}

.workflow-text-to-video-node__model-row {
  display: grid;
...
.workflow-text-to-video-node__tip {
  display: inline-flex;
  align-items: center;
...
.workflow-text-to-video-node__switch-row strong,
.workflow-text-to-video-node__switch-row small {
  display: block;
}
