...
const firstFrameCascaderRef = ref<InstanceType<typeof NodeCascader>>();
...
function arrayValue(value: unknown, fallback: unknown[] = []) {
  return Array.isArray(value) ? cloneDeep(value) : cloneDeep(fallback);
}
...
function normalizeNodeData(value: unknown): NodeData {
  ...
  return {
    ...
    first_frame_url: arrayValue(source.first_frame_url, [
      'start-node',
      'image',
    ]),
    is_result: typeof source.is_result === 'boolean' ? source.is_result : true,
    last_frame_url: arrayValue(source.last_frame_url),
    model_id: modelIdValue(sourceModelId),
    model_id_reference: arrayValue(source.model_id_reference),
    model_id_type:
...
async function validate() {
  try {
    if (formData.value.model_id_type === 'reference') {
      ...
      throw validationError('请选择图生视频模型');
    }

    if (!hasReferenceValue(formData.value.first_frame_url)) {
      throw validationError('请选择首帧图片');
    }
    await firstFrameCascaderRef.value?.validate?.();
...
<template>
  ...
  >
    <ElForm
      :model="formData"
      class="workflow-image-to-video-node"
      label-position="top"
      @submit.prevent
    >
      <section class="workflow-image-to-video-node__panel">
        <ElFormItem label="图生视频模型" required>
          <template #label>
            <div class="workflow-image-to-video-node__form-label">
              <span>图生视频模型</span>
              <ElSelect
          ...
          <div class="workflow-image-to-video-node__model-row">
            <LocalModelSelect
        ...
        <ElFormItem label="提示词" required>
          <template #label>
            <div class="workflow-image-to-video-node__form-label">
              <span class="workflow-image-to-video-node__label-with-tip">
                提示词
                <ElTooltip
                  content="描述要生成的视频内容，支持 {{节点.字段}} 引用。"
                  placement="right"
                >
                  <span class="workflow-image-to-video-node__tip">?</span>
                </ElTooltip>
              </span>
              <ElButton
                size="small"
      ...
      <section class="workflow-image-to-video-node__panel">
        <ElFormItem label="首帧图片" required>
          <template #label>
            <div class="workflow-image-to-video-node__form-label is-start">
              <span class="workflow-image-to-video-node__label-with-tip">
                首帧图片
                <FileTooltip />
              </span>
          ...
          <NodeCascader
            ...
            :model-value="formData.first_frame_url"
            :node-model="nodeModel"
            class="w-full"
            placeholder="请选择首帧图片变量"
            @update:model-value="patchData('first_frame_url', $event)"
          />
        </ElFormItem>

        <ElFormItem label="尾帧图片">
          <template #label>
            <div class="workflow-image-to-video-node__form-label is-start">
              <span class="workflow-image-to-video-node__label-with-tip">
                尾帧图片
          ...
          <NodeCascader
            :model-value="formData.last_frame_url"
            :node-model="nodeModel"
            class="w-full"
            clearable
            placeholder="可选，选择尾帧图片变量"
            @update:model-value="patchData('last_frame_url', $event)"
          />
        </ElFormItem>
      </section>
      ...
      <section
        ...
        class="workflow-image-to-video-node__panel"
      >
        <div class="workflow-image-to-video-node__switch-row">
          <div>
            <strong>返回内容</strong>
            <small>将生成视频作为工作流输出内容</small>
          </div>
          <ElSwitch
...
.workflow-image-to-video-node,
.workflow-image-to-video-node__panel {
  display: grid;
  gap: 6px;
}
...
.workflow-image-to-video-node :deep(.el-form-item) {
  margin-bottom: 6px;
}

.workflow-image-to-video-node :deep(.el-form-item:last-child) {
  margin-bottom: 0;
}

.workflow-image-to-video-node :deep(.el-form-item__label) {
  margin-bottom: 2px;
  font-size: var(--el-font-size-extra-small);
  line-height: 16px;
}

.workflow-image-to-video-node :deep(.el-input__wrapper) {
  min-height: 28px;
}

.workflow-image-to-video-node :deep(.el-textarea__inner) {
  min-height: 0 !important;
  padding-block: 4px;
  line-height: 18px;
}

.workflow-image-to-video-node__form-label,
.workflow-image-to-video-node__label-with-tip,
.workflow-image-to-video-node__switch-row {
  display: flex;
  gap: 6px;
  align-items: center;
  min-width: 0;
...
.workflow-image-to-video-node__form-label.is-start {
  justify-content: flex-start;
}

.workflow-image-to-video-node__form-label .el-select {
  width: 92px;
}

.workflow-image-to-video-node__model-row {
  display: grid;
...
.workflow-image-to-video-node__tip {
  display: inline-flex;
  align-items: center;
...
.workflow-image-to-video-node__switch-row strong,
.workflow-image-to-video-node__switch-row small {
  display: block;
}
